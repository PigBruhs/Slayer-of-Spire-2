...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input", default="config/auto_menu_mouse.example.json", help="Template JSON to load")
    parser.add_argument("--output", default="config/auto_menu_mouse.local.json", help="Output JSON path")
    parser.add_argument("--only", default="", help="Comma-separated sequence names to capture")
    ...
    parser.add_argument("--game-relative", action="store_true", help="Save coordinates relative to game window (gx/gy or grx/gry)")
...
def main() -> None:
    ...
    if not isinstance(sequences, dict):
        raise RuntimeError("invalid config: missing 'sequences' object")

    selected = _selected_names(sequences, args.only)
    if not selected:
    ...
    for seq_name in selected:
        ...
        print(f"\n=== Sequence: {seq_name} ({len(steps)} steps) ===")
...
def _selected_names(sequences: dict[str, object], only: str) -> list[str]:
    if not only.strip():
        return [name for name in sequences.keys() if isinstance(name, str)]
    wanted = {part.strip() for part in only.split(",") if part.strip()}
    return [name for name in sequences.keys() if isinstance(name, str) and name in wanted]
...
def get_game_window_rect(process_name: str) -> tuple[int, int, int, int] | None:
    ...
    rect = RECT()
    ok = user32.GetWindowRect(found[0], ctypes.byref(rect))
    if not ok:
        return None
